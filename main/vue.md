...
![mvvm](https://zh.wikipedia.org/wiki/MVVM#/media/File:MVVMPattern.png)
...
Vue 3 diff 对比过程如下：
1. **同层级比较** 同类型节点继续以下步骤，不同类型节点直接替换。
...
3. **静态节点提升不被diff,生成Block Tree**

4. **动态节点对比**

   无key的列表使用顺序对比.
 
   对有key的节点列表使用 **双端对比算法**，即从两端同时开始比较：
    - 首先比较头节点和尾节点。
    - 如果无法匹配，则逐步从中间寻找匹配项。
    - 当两端指针重叠时，停止比较。

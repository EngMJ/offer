...
1. **组件层面**
    ...
    - 如果是组件 VNode，更新 props、触发组件内部 render 生成子树，再递归进入 patch。
...
3. **属性层面**
    - 比较旧 VNode 和新 VNode 的 data 对象。
    - 针对不同的属性（如 class、style、事件、其他自定义属性），分别调用对应更新 API 修改真实 DOM 元素。

4. **子节点层面**
    - 如果子节点为文本，则直接比较更新文本内容。
    - 如果子节点为数组，则进入 diff 流程：
        - 先进行头部和尾部同步，快速处理连续相同部分；
        - 对中间部分使用 key 映射、构造索引映射数组，然后利用最长递增子序列算法找出无需移动的部分；
        - 对于新增、移动、删除的节点分别执行创建、插入、删除操作。

...
// 生成映射数组：每个元素代表 newNodes 中节点在 oldNodes 中的索引
const mapping = newNodes.map(key => oldNodes.indexOf(key));
// 结果：mapping = [1, 0, 4, 2, 3]
// 含义是：
// 新数组中第一个节点 'B' 对应旧数组中索引为 1 的位置。
// 新数组中第二个节点 'A' 对应旧数组中索引为 0 的位置。
// …以此类推
```
...
使用“贪心 + 二分”来计算最长递增子序列，其时间复杂度通常为 O(n log n)。

+ **为什么最长递增子序列中的节点不需要移动呢？**

为尽量减少 DOM 操作,算法需要找出哪些节点的位置其实已经是正确的，不需要再移动，而哪些节点需要插入或移动。

**相对正确的顺序:** 通过计算 newIndexToOldIndexMap 数组中的最长递增子序列， 首先说明这些节点在旧列表中也是按照新列表的顺序排列的。换句话说，这部分节点相对于彼此来说已经处于正确的顺序，不需要移动。

**最小移动的次数:** 从数学角度讲，移动操作的最小次数可以由“总节点数减去最长递增子序列的长度”得到。也就是说，保持最长递增子序列不动，其余节点都需要调整，这正是一个最优化的解法。

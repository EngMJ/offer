...
- **Vite**：
    - **开发服务器启动速度快**：Vite 使用原生 ES 模块（ESM）直接在浏览器中加载模块，并且使用` esbuild 预构建依赖`,`按需加载开发代码`。
...
| 特性             | Vite                | Webpack               |
|----------------|---------------------|-----------------------|
| **开发启动速度**     | 快速，依赖预构建，按需编译，适合大型项目 | 较慢，需要打包整个项目           |
| **热更新速度**      | 快，基于原生 ESM,按需更新     | 较慢，尤其在大型项目中, 重新打包整个模块 |

...
**作用:**
1. `转换为响应式ref`: 可以将 值 / refs / getters 规范化为 ref (3.3+)。
2. `创建属性`: 可为ref / reactive 不存在属性使用toRef(),依然返回ref对象.
3. `值关联`: 修改 toRef 后的响应式值,原始值也会发生变化, 反之亦然.
4. `参数传递保持值响应性`: 主要作为props等数据传递给组合式函数使用依然保持响应式连接.
...
**作用:**
1. `所有属性值关联`:为响应式对象每个可枚举的属性创建数据连接.
2. `参数传递保持响应式`:主要用于组合式函数的参数传递.
3. `解构后依然保持响应式`: toRefs后的对象解构后依然保持响应式连接.
...
**作用:**
1. `自动运行不同方式获取值`: 针对值、refs 或 getters 进行不同方式(直接返回/解包/运行函数)获取其值。
2. `规范化参数,始终传递值`: 可在组合式函数中使用，传递规范化的值、ref 或 getter 的参数。

Markdown 指南中文版 Markdown 指南中文版
入门指南 速查表 基本语法 扩展语法 Hacks 书写工具 相关书籍
Markdown 基本语法
...
放在方括号内的标签，以及紧跟在方括号后面的一个冒号和至少一个空格（例如 [label]: ）。
...
链接（Link）的最佳使用实践
不同的 Markdown 应用程序在处理 URL 中间的空格方面是不一样的。为了兼容起见，请尽量使用 %20 （空格的编码形式）来代替空格。或者，如果你的 Markdown 应用程序 支持 HTML 的话，你可以使用 HTML 的 a 标签。
...
注意： 要了解如何调整图片大小，请参阅 图片尺寸 章节。要了解图和添加标题，请参阅 section on 图片标题 章节。
带链接的图片
